...
def collect_answers(config_path: str, results_dir: Path):

    config = BenchmarkConfig.load(config_path)
    benchmark = BMRS(config)

    ask_model = model_setup()
    results_path = results_dir / config.model / "vlm_results.json"

    results = benchmark.run(
        ask_model,
        reload_model,
        checkpoint_dir = str(RESULTS_DIR / config.model / "checkpoints"),
        strategies=[
            StrategyName.DESCRIPTIVE_ITERATIVE,
            StrategyName.CONTRASTIVE_DIRECT,
        ],
    )
    results.save_as_json(results_path)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()

    parser.add_argument("--config_path", type=str, default="configs/test_config.json")
    ...
    collect_answers(config_path, results)